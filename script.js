            ...
            '宇嫺': {
                ...
                positiveBehaviors: [
                    { 
                        id: 1, 
                        name: '主動做作業', 
                        description: '沒有提醒的情況下，自己安排時間完成作業', 
                        points: 2, 
                        tags: ['學習', '責任'] 
                    },
                    { 
        ...
        let currentChild = localStorage.getItem('currentChild') || '宇嫺';
        ...
        // Modal元素
        const addPositiveModal = new bootstrap.Modal(document.getElementById('addPositiveModal'));
        const addNegativeModal = new bootstrap.Modal(document.getElementById('addNegativeModal'));
        const exchangeModal = new bootstrap.Modal(document.getElementById('exchangeModal'));
        const editPositiveModal = new bootstrap.Modal(document.getElementById('editPositiveModal'));
        const editNegativeModal = new bootstrap.Modal(document.getElementById('editNegativeModal'));
        ...
        function initializePage() {
            if (!checkLocalStorage()) {
                alert('警告：本應用需要瀏覽器的 LocalStorage 支持。請確保您沒有禁用 LocalStorage，並且不在隱私模式下運行。');
            }
            loadChildData();
            ...
            // 監聽儲存正向行為按鈕
            document.getElementById('savePositiveBtn').addEventListener('click', function() {
                ...
                // 收集標籤
                const tags = [];
                if (document.getElementById('positiveTagLearning').checked) tags.push('學習');
                if (document.getElementById('positiveTagHealth').checked) tags.push('健康');
                if (document.getElementById('positiveTagResponsibility').checked) tags.push('責任');
                if (document.getElementById('positiveTagHabits').checked) tags.push('生活習慣');
            ...
            // 監聽儲存需改善行為按鈕
            document.getElementById('saveNegativeBtn').addEventListener('click', function() {
                ...
                // 收集標籤
                const tags = [];
                if (document.getElementById('negativeTagLearning').checked) tags.push('學習');
                if (document.getElementById('negativeTagHealth').checked) tags.push('健康');
                if (document.getElementById('negativeTagResponsibility').checked) tags.push('責任');
                if (document.getElementById('negativeTagHabits').checked) tags.push('生活習慣');
            ...
            document.getElementById('confirmExchangeBtn').addEventListener('click', function() {
                ...
                if (name && exchangePoints) {
                    if (exchangePoints <= childrenData[currentChild].points) {
                        ...
                        renderPointsCard();
                        exchangeModal.hide();
                        alert(`成功兌換「${name}」(${category})，消耗了 ${exchangePoints} 點數！`);
                    } else {
            ...
            // 監聽更新正向行為按鈕
            document.getElementById('updatePositiveBtn').addEventListener('click', function() {
                const id = parseInt(document.getElementById('editPositiveBehaviorId').value);
                const name = document.getElementById('editPositiveBehaviorName').value;
                const description = document.getElementById('editPositiveBehaviorDesc').value;
                const points = parseInt(document.getElementById('editPositivePoints').value);
                
                // 收集標籤
                const tags = [];
                if (document.getElementById('editPositiveTagLearning').checked) tags.push('學習');
                if (document.getElementById('editPositiveTagHealth').checked) tags.push('健康');
                if (document.getElementById('editPositiveTagResponsibility').checked) tags.push('責任');
                if (document.getElementById('editPositiveTagHabits').checked) tags.push('生活習慣');
                
                if (name && points) {
                    const behaviorIndex = childrenData[currentChild].positiveBehaviors.findIndex(b => b.id === id);
                    if (behaviorIndex !== -1) {
            ...
            // 監聽更新需改善行為按鈕
            document.getElementById('updateNegativeBtn').addEventListener('click', function() {
                const id = parseInt(document.getElementById('editNegativeBehaviorId').value);
                const name = document.getElementById('editNegativeBehaviorName').value;
                const description = document.getElementById('editNegativeBehaviorDesc').value;
                const points = parseInt(document.getElementById('editNegativePoints').value);
                
                // 收集標籤
                const tags = [];
                if (document.getElementById('editNegativeTagLearning').checked) tags.push('學習');
                if (document.getElementById('editNegativeTagHealth').checked) tags.push('健康');
                if (document.getElementById('editNegativeTagResponsibility').checked) tags.push('責任');
                if (document.getElementById('editNegativeTagHabits').checked) tags.push('生活習慣');
                
                if (name && points) {
                    const behaviorIndex = childrenData[currentChild].negativeBehaviors.findIndex(b => b.id === id);
                    if (behaviorIndex !== -1) {
        ...
        function renderPointsCard() {
            ...
            // 計算總點數和總頁數
            const totalPoints = Math.max(15, Math.ceil(childrenData[currentChild].points / 5) * 5);
                ...
                card.innerHTML = `
                    <div class="d-flex justify-content-between align-items-start">
                        <div class="behavior-content" style="cursor: pointer">
                            ...
                            <div class="text-muted small">${behavior.description || ''}</div>
                            ${tagsHtml}
                        </div>
                        <div class="d-flex flex-column align-items-end">
                            <span class="badge bg-success mb-2">+${behavior.points} 點</span>
                            <div class="btn-group btn-group-sm">
                                <button class="btn btn-outline-secondary btn-edit" data-id="${behavior.id}">編輯</button>
                                <button class="btn btn-outline-danger btn-delete" data-id="${behavior.id}">刪除</button>
                            </div>
                        </div>
                    </div>
                `;
        ...
        function openEditPositiveModal(behavior) {
            ...
            document.getElementById('editPositiveBehaviorDesc').value = behavior.description || '';
            document.getElementById('editPositivePoints').value = behavior.points;
            
            // 重置所有標籤
            document.getElementById('editPositiveTagLearning').checked = behavior.tags.includes('學習');
            document.getElementById('editPositiveTagHealth').checked = behavior.tags.includes('健康');
            document.getElementById('editPositiveTagResponsibility').checked = behavior.tags.includes('責任');
            document.getElementById('editPositiveTagHabits').checked = behavior.tags.includes('生活習慣');
            
            editPositiveModal.show();
        }

        function deletePositiveBehavior(id) {
            childrenData[currentChild].positiveBehaviors = childrenData[currentChild].positiveBehaviors.filter(b => b.id !== id);
            renderPositiveBehaviors();
        }
            ...
            childrenData[currentChild].negativeBehaviors.forEach(behavior => {
                ...
                card.innerHTML = `
                    <div class="d-flex justify-content-between align-items-start">
                        ...
                        <div class="d-flex flex-column align-items-end">
                            <span class="badge bg-danger mb-2">-${behavior.points} 點</span>
                            <div class="btn-group btn-group-sm">
                                <button class="btn btn-outline-secondary btn-edit" data-id="${behavior.id}">編輯</button>
                                <button class="btn btn-outline-danger btn-delete" data-id="${behavior.id}">刪除</button>
                            </div>
        ...
        function openEditNegativeModal(behavior) {
            ...
            document.getElementById('editNegativeBehaviorDesc').value = behavior.description || '';
            document.getElementById('editNegativePoints').value = behavior.points;
            
            // 重置所有標籤
            document.getElementById('editNegativeTagLearning').checked = behavior.tags.includes('學習');
            document.getElementById('editNegativeTagHealth').checked = behavior.tags.includes('健康');
            document.getElementById('editNegativeTagResponsibility').checked = behavior.tags.includes('責任');
            document.getElementById('editNegativeTagHabits').checked = behavior.tags.includes('生活習慣');
            
            editNegativeModal.show();
        }

        function deleteNegativeBehavior(id) {
            childrenData[currentChild].negativeBehaviors = childrenData[currentChild].negativeBehaviors.filter(b => b.id !== id);
            renderNegativeBehaviors();
        }
